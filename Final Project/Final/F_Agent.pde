...
class Agent2 {
  ...
  //agent constructor
  Agent2(float x, float y, int rad, float maxS, ArrayList<Path> pathArray, int k_id) {
    isAlive = true;
    r = rad;
    tolerance *= r;
    maxspeed = maxS;
    ...
    //starts from the reverse direction, so pathIndex is the maximum. pathIndex will increment down to 0. 
    pathDirection = 1; 
    /*if (random(-1, 1) <= 0 ) { //path direction will always be 1
      pathDirection = -1;
    } else {
      pathDirection = +1;
    }
  ...
  void update(ArrayList<PVector> others, boolean collisionDetection) {
    ...
    // Checks if Agents reached current waypoint
    // If reaches endpoint, DO NOT reverse direction
    //
    float prox = sqrt( sq(location.x - waypoint.x) + sq(location.y - waypoint.y) );
    if (prox < 3 && path.size() > 1 ) {
      if (pathIndex == pathLength - 1) {
        ...
        // If back to the beginning, move on to the next path
          ...
          pathArrayIndex_mrf++;
          if (pathArrayIndex_mrf < pathArray.size()) {
            path = pathArray.get(pathArrayIndex_mrf).waypoints; //get 1, 2 path within pathArray
            pathToDraw = pathArray.get(pathArrayIndex_mrf);
            pathLength = path.size();
            pathIndex = getClosestWaypoint(location); 
          } else {
